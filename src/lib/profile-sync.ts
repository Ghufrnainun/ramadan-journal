...
import { getProfile, saveProfile, UserProfile } from '@/lib/storage';
  ...
  const mapDbToProfile = (row: any): Partial<UserProfile> => ({
    language: row.language || 'id',
    location: row.city && row.province ? { city: row.city, province: row.province } : null,
    ramadanStartDate: row.ramadan_start_date || null,
    ramadanEndDate: row.ramadan_end_date || null,
    focusModules: row.focus_modules || ['prayer', 'quran', 'dhikr', 'tracker', 'reflection'],
    reminders: {
...
export const syncProfileOnLogin = async (userId: string) => {
  ...
  if (!data) {
    await supabase.from('profiles').update(mapProfileToDb(local)).eq('user_id', userId);
    return;
  }
...
export const saveProfileAndSync = async (updates: Partial<UserProfile>, userId?: string | null) => {
  saveProfile(updates);
  if (!userId) return;
  const profile = getProfile();
  const { error } = await supabase.from('profiles').update(mapProfileToDb(profile)).eq('user_id', userId);
  if (error) {
