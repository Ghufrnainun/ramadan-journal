...
const content = {
  id: {
    ...
    fetchError: 'Gagal memuat hadits. Coba lagi nanti.',
  },
  en: {
    title: 'Arbain Hadith',
    ...
    fetchError: 'Failed to load hadith. Please try again later.',
  },
};
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        console.error('Failed loading hadith:', error);
        setHadiths([]);
        setLoadError(
          profile.language === 'id'
            ? 'Gagal memuat hadits. Coba lagi nanti.'
            : 'Failed to load hadith. Please try again later.',
        );
      } finally {
        setIsLoading(false);
      }
      ...
      <div className={cn('md:p-0', !selectedHadith && 'px-6')}>
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {loadError && (
              <div className="col-span-full rounded-xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-200">
                {loadError || t.fetchError}
              </div>
            )}
            {!loadError &&
            ...
            {!loadError && filteredHadiths.length === 0 && (
